...
internal class DuckDBTest
{
    public DuckDBTest()
    {
        ...
        {
            builder.RegisterType<DBContext>().Keyed<IDBContext>(TableMode.DUCKDB);
        };
    ...
    public async Task QueryDataAsync(IDataTable table)
    {
        ...
        var setting = new DuckDBQueryFilter()
        {
            Logic = LogicMode.IN,
        ...
        var settingDieX = new DuckDBQueryFilter
        {
    ...
    public async Task MergeColumnsTest()
    {
        ...
        setting.Join = JoinMode.INNER_JOIN;
